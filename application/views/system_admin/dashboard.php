...
<div class="card shadow mb-4">
    <div class="py-3 d-flex flex-row align-items-center justify-content-between">
        <h5 class="m-0 font-weight-bold text-primary-soft ml-4">
            <i class="fas fa-eye me-2"></i>จำนวนผู้เข้าชม ( ทั้งหมด )
        </h5>
    </div>
    <div class="card-body">
            ...
            <div class="visitor-progress progress mb-3">
                <div class="progress-bar views" style="width: 100%;">
                    <div class="progress-text">
                        <span class="member-name"><?= $table->table_name; ?></span>
...
<div class="card shadow mb-4">
    <div class="py-3 d-flex flex-row align-items-center justify-content-between">
        <h5 class="m-0 font-weight-bold text-primary-soft ml-4">
            <i class="fas fa-file-alt me-2"></i>จำนวนกระทู้ ( ทั้งหมด )
        </h5>
    </div>
    <div class="card-body">
...
// Debug: ตรวจสอบข้อมูลจาก PHP
console.log('PHP Data Check:', <?php echo json_encode($top_active_users); ?>);
...
// Debug: ตรวจสอบว่ากราฟถูกสร้างหรือไม่
console.log('Chart container:', document.querySelector("#topActiveUsersChart"));
console.log('Chart data:', userData);
console.log('Chart names:', userNames);
console.log('Chart rendered:', ultraChart);

// ตรวจสอบว่า container มีอยู่จริงและสร้างกราฟใหม่หากจำเป็น
setTimeout(() => {
    const container = document.querySelector("#topActiveUsersChart");
    if (container && userData.length === 0) {
        container.innerHTML = '<div class="text-center py-4"><p class="text-muted">ไม่มีข้อมูลในช่วงเวลานี้</p></div>';
    }
}, 1000);

<?php else: ?>
// Debug: No data available
console.log('No top_active_users data available');
<?php endif; ?>

// ========== TIME SELECTOR FUNCTION ========== 
function selectTimeRange(button, days) {
    console.log('selectTimeRange called with days:', days);
    ...
    .then(data => {
        ...
        if (data.success && data.data && data.data.length > 0) {
            ...
            if (typeof ultraChart !== 'undefined') {
                ...
                console.error('ultraChart is not defined');
            }
            
            <?php endif; ?>
        } else {
            console.log('No data or empty response:', data);
