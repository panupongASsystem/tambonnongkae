...
<?php
                ...
                // คำนวณการแบ่งคอลัมน์
                $total_items = count($all_structure_items);
                $columns_count = 3;
                $items_per_column = ceil($total_items / $columns_count);

                // แบ่งรายการออกเป็นคอลัมน์
                $structure_columns = [];
                $all_structure_items = array_values($all_structure_items); // Reset array keys

                for ($col = 0; $col < $columns_count; $col++) {
                    $structure_columns[$col] = [];
                    $start_index = $col * $items_per_column;
                    $end_index = min($start_index + $items_per_column, $total_items);

                    for ($i = $start_index; $i < $end_index; $i++) {
                        if (isset($all_structure_items[$i])) {
                            $structure_columns[$col][] = $all_structure_items[$i];
                        }
                    }
                }
                ?>
...
<svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <div class="d-flex justify-content-start" style="position: absolute; margin: 25px;">
      ...
      <div style="z-index: 101; margin-right: 20px;">
          <img style="margin-left: 10px;" src="<?php echo base_url('docs/logo-nav.png'); ?>" width="120" height="120">
      </div>
