                    ...
                    <?php if(isset($members) && count($members) > 0): ?>
                        <?php foreach ($members as $member): ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <img src="<?php echo base_url('docs/' . (!empty($member->mp_img) ? 'img/avatar/' . $member->mp_img : 'default_user.png')); ?>" 
...
window.viewMemberDetails = function(member) {
    ...
    const profileImg = member.mp_img 
        ? `${site_url}docs/img/avatar/${member.mp_img}` 
        : `${site_url}docs/img/avatar/default_user.png`;
