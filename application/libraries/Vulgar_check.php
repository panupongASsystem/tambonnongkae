...
public function check_form($form_data, $fields_to_check = null)
{
        ...
        if (isset($form_data[$field]) && !empty($form_data[$field])) {
            ...
            if (!in_array($field, ['q_a_email', 'q_a_reply_email'])) {
                ...
                if (!$this->check_no_urls($form_data[$field])) {
                    ...
                    log_message('warning', 'check_form: URL detected in field "' . $field . '": ' . $form_data[$field]);
    ...
    if ($has_url) {
        log_message('warning', 'check_form: URLs detected in fields: ' . implode(', ', $url_detected_fields));
    }
    ...
    public function check_text($text)
    {
        ...
        if (empty($text)) {
            log_message('warning', 'check_text: Input text is empty.');
    ...
    private function _check_with_api($text)
    {
        ...
        if (isset($result['data']['has_vulgar_words']) && $result['data']['has_vulgar_words']) {
            ...
            log_message('warning', '_check_with_api: API detected vulgar words: ' . json_encode($vulgar_words, JSON_UNESCAPED_UNICODE));
            log_message('warning', '_check_with_api: Original text: "' . $text . '"');
            log_message('warning', '_check_with_api: Full API response: ' . json_encode($result, JSON_UNESCAPED_UNICODE));
        }
    ...
    foreach ($url_patterns as $name => $pattern) {
        if (preg_match($pattern, $text_without_emails)) {
            ...
            if (isset($this->CI->form_validation)) {
                ...
                log_message('warning', 'check_no_urls: Form validation not loaded, cannot set validation message');
            }
